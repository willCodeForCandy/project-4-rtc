export const myProjects = [
  {
    title: 'Kawaii Weather',
    description:
      'Una aplicación web del clima, con estética kawaii. Fue mi primer proyecto con React, usando la api de OpenWeather para obtener la información del clima. Busca el estado del tiempo en tu ubicación actual y permite buscar otras ubicaciones por nombre y guardar una lista de favoritos.',
    screenshot: '/assets/kawaii-weather.png',
    technologies: ['html', 'css', 'js', 'React'],
    imgDescription:
      'Una captura de pantalla de una aplicación que muestra el clima de distintas ciudades, con fondos en color pastel e imágenes con caritas sonrientes.',
    gitHubUrl: 'https://github.com/willCodeForCandy/project-11bis',
    liveUrl: 'https://kawaii2-0.vercel.app/',
  },
  {
    title: 'The Meeple King',
    description:
      'Una aplicación web para organizar quedadas para jugar juegos de mesa. La información de los juegos es de <a href="https://boardgamegeek.com/">BGG</a>. Este fue mi primer proyecto fullstack. Hice el backend con Node.js y Express, usando MongoDB.',
    screenshot: '/assets/meeple-king.png',
    technologies: ['html', 'css', 'js', 'Node.js'],
    imgDescription:
      'Screenshot landing page The Meeple King. En la parte superior, un banner con texto y un botón para crear eventos, y debajo un listado de próximos eventos.',
    gitHubUrl: 'https://github.com/willCodeForCandy/project-10-frontend',
    liveUrl: 'https://project-10-frontend.vercel.app/',
  },
  {
    title: 'Games Hub',
    description:
      'Una web con 3 juegos codeados desde cero con js. Definitivamente la UI del rock, paper, scissors, lizard, Spock podría mejorarse.',
    screenshot: '/assets/games-hub.png',
    technologies: ['html', 'css', 'js'],
    imgDescription: 'Screenshot memotest',
    gitHubUrl: 'https://github.com/willCodeForCandy/games-hub',
    liveUrl: 'https://viru-games-hub.netlify.app/',
  },
  {
    title: 'Rockterest',
    description:
      '¡Primer proyecto con async! En este proyecto, usé la API de Unsplash para crear una página similar a Pinterest que cargue las imágenes de forma dinámica y utilice el motor de búsqueda de Unsplash.<br>PD: si buscas algo muy, muy raro, tal vez encuentres una sorpresa 😉',
    screenshot: '/assets/rockterest.png',
    technologies: ['html', 'css', 'js'],
    imgDescription:
      'Una galería con imágenes de bosques, con una barra de búsqueda y navegación en la parte superior.',
    gitHubUrl: 'https://github.com/willCodeForCandy/project-3-pinterest',
    liveUrl: 'https://rockterest.netlify.app/',
  },
  {
    title: 'Tienda con filtro',
    description:
      'Uno de mis primeros proyectos con js. El objetivo era crear una tienda de manera dinámica a partir de un mock de base de datos y luego agregar filtros de búsqueda. <br> En este caso, la víctima fue PC Componentes.',
    screenshot: '/assets/proyecto-2.png',
    technologies: ['html', 'css', 'js'],
    imgDescription: 'Screenshot tienda con filtro desktop',
    gitHubUrl: 'https://github.com/willCodeForCandy/proyecto-prework-2',
    liveUrl: 'https://proyecto2pc-componentes.netlify.app/',
  },
  {
    title: 'Tienda con filtro v2',
    description:
      'La estética de PC Componentes me resultó poco inspiradora, por lo que en el segundo intento imaginé una tienda de arte para comprar prints directamente a lxs artistas. Una estética relajada con colores agradables fue el foco en este proyecto, conservando la funcionalidad del filtro.',
    screenshot: '/assets/galeria-con-filtro.png',
    technologies: ['html', 'css', 'js'],
    imgDescription: 'Screenshot tienda de arte con filtro mobile',
    gitHubUrl: '',
    liveUrl: 'https://tpmba-gallery-project.netlify.app/',
  },
  {
    title: 'Selector de colores',
    description:
      'Con este pequeño ejercicio puse en práctica el uso de Vite y js DOM.<br>Con esta aplicación se puede elegir un color predeterminado de una lista o bien de una paleta y ver cómo queda en el fondo y cuál es su código HEX.',
    screenshot: '/assets/color-picker.png',
    technologies: ['html', 'css', 'js'],
    imgDescription: 'Screenshot ejercicio color picker mobile',
    gitHubUrl: 'https://github.com/willCodeForCandy/color-flipper',
    liveUrl: 'https://colorpickerexercise.netlify.app/',
  },
  {
    title: 'Password checker',
    description:
      'Esta mini app desenfoca la imagen de fondo dependiendo del largo de la contraseña.',
    screenshot: '/assets/password-checker.png',
    technologies: ['html', 'css', 'js'],
    imgDescription: 'Screenshot ejercicio password mobile',
    gitHubUrl: 'https://github.com/willCodeForCandy/bluring',
    liveUrl: 'https://buring-passcode.netlify.app/',
  },
  {
    title: 'Landing page',
    description:
      'Mi primer proyecto para Rock{The Code} fue un clon de la web de Imdb, usando únicamente HTML y CSS.<br>El mayor reto en este proyecto fue la creación de elementos dinámicos como un carousel y un menú hamburguesa.<br> Fun fact: la primera vez que me pidieron hacer un menú hamburguesa, pensé que se trataba de un menú para una hamburguesería y entregué un par de listas con títulos y un fondo de hamburguesas.',
    screenshot: '/assets/clon-imdb.png',
    technologies: ['html', 'css'],
    imgDescription: 'Screenshot clon Imdb desktop',
    gitHubUrl: 'https://github.com/willCodeForCandy/ejemplos-git-pmba',
    liveUrl: 'https://clon-imdb.netlify.app/',
  },
  {
    title: 'Landing page God Of War',
    description:
      'Primer proyecto, v2. Reproduje un diseño de Figma que me pareció atractivo.',
    screenshot: '/assets/gow-landing.png',
    technologies: ['html', 'css'],
    imgDescription: 'Screenshot landing GOW',
    gitHubUrl: 'https://github.com/willCodeForCandy/Project-1-GOW',
    liveUrl: 'https://project-1-gow.netlify.app/',
  },
];
